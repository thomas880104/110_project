...
testWAVs= pd.read_csv(basePath+'test.txt', sep=" ", header=None, error_bad_lines=False)[0].tolist()
valWAVs=  pd.read_csv(basePath+'validation.txt', sep=" ", header=None, error_bad_lines=False)[0].tolist()


testWAVs= [os.path.join(basePath+'test/', f) for f in testWAVs if f.endswith('.wav')]
valWAVs=  [os.path.join(basePath+'validation/', f) for f in valWAVs if f.endswith('.wav')]
...
ryGscDict=  {   'unknown' : 0, 'silence' : 0, 
                ...
                '零' : 1 , '一' : 2 , '二' : 3 , '三' : 4 , '四' : 5 ,
                '五' : 6 , '六' : 7 , '七' : 8 , '八' : 9 , '九' : 10 ,
                '十' : 11 ,'後退' : 12 ,'前進' : 13 ,'上' : 14 ,'下' : 15 ,
                '左' : 16 ,'右' : 17 ,'床' : 18 ,'去' : 19 ,'快樂' : 20 ,
                '房屋' : 21 ,'樹' : 22 ,'鳥' : 23 ,'貓' : 24 ,'狗' : 25 ,
                '志明' : 26 ,'春嬌' : 27 ,'可以' : 28 ,'不可' : 29 ,'開' : 30 ,
                '關' : 31
                 }
